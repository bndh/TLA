...
(async () => {
	try {
		...
		const data = await rest.put(
			Routes.applicationGuildCommands(process.env.CLIENT_ID, process.env.GUILD_ID), // Change to Routes.applicationCommands(process.env.CLIENT_ID) for global commands
			{body: commands}
		);
